...
A Lucky Break trigger adds a random one to five points, so it can roll one. Excess progress is discarded when the goal completes. Lucky Break also applies only to manual mining, not Chunk Drills.

## I Cannot Invite Someone to a Co-op

- The other player must be online.
- Neither player can already be in another co-op.
- Both players need enough Co-op Size for the resulting group.
- The invitation expires after 60 seconds.

If a reset just occurred or someone left the world, create the co-op again.
...
The collector needs an empty bucket in your MasterChest for every water or lava source block. It does not collect flowing fluids. Confirm that the upgrade is enabled in `/chunkdrill` and that the network has room for the filled bucket.

## My Drill Disappeared
