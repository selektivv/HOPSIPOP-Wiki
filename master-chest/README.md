# Master Chest

The Master Chest is your personal, searchable storage network. It stores items virtually, so every Master Chest access point you own shows the same contents. You can open the network with a command, carry a Master Chest as a portable terminal, or place several access points around your base.

## What It Does

- Combines your stored items into one network.
- Groups matching items and shows their total amounts.
- Sorts items into familiar Minecraft categories.
- Lets you deposit and retrieve items directly from a menu.
- Supports trusted-player access and shared networks.
- Supports automatic restocking and network-assisted crafting through OmniSync.
- Supports crafting, stone-cutting, smelting, and deletion jobs.
- Connects to hoppers for automatic input and output.

## Recommended Reading Order

1. [Getting Started](getting-started.md)
2. [Storing and Retrieving Items](storing-and-retrieving.md)
3. [Capacity and Progression](capacity-and-progression.md)
4. [OmniSync](omnisync.md)
5. [Automation Jobs](automation.md)

## Main Commands

| Command | Purpose |
| --- | --- |
| `/mc` | Open the network currently selected as your default. |
| `/masterchest` | Open the Master Chest in the same way as `/mc`. |
| `/roadmap` | View progression requirements and feature unlocks. |
| `/playtime` | View playtime, first join date, and current item capacity. |
| `/mc guides` | Browse in-game guide books. |
| `/mc jobs` | Open the automation menu. |
| `/shortcuts` | Open the Master Chest shortcuts menu. |
| `/mc shared` | Browse networks that other players have shared with you. |
