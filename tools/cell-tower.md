...
- **Cyberpunk:** An asymmetric megacity relay with exposed neon service rails, fragmented hologram rings, luminous junction nodes, offset billboard fins, and a split antenna crown.

Default is always available. The eight alternative designs are unlocked separately for each tool and player by an operator with `/masterchest theme unlock <player> tower <cherry|copper|mangrove|lapis|lush|medieval|quartz|cyberpunk>`. A locked design can be tried for two minutes by clicking it. After the preview ends, the tower returns to Default and that design enters a one-hour preview cooldown.
