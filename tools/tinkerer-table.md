...
Default is always available. The seven alternative designs are unlocked separately for each tool and player by an operator with `/masterchest theme unlock <player> tinkerer <cherry|copper|mangrove|lapis|lush|medieval|quartz>`. A locked design can be tried for two minutes by clicking it. After the preview ends, the table returns to Default and that design enters a one-hour preview cooldown.
